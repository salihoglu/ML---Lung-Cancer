...
PPI_TREE_RELATIVE = os.path.join(
    "bioinf_scripts", "scripts", "ppi_network", "tables", "node_centrality.csv"
)
...
def infer_root(project_root: str, manifest_paths: List[str]) -> str:
    ...
    for p in manifest_paths:
        m = re.search(r"/(TCGA_GDC_download|GEO_PREP/", p)
        if m:
            return p[:m.start()]
    return direct
...
def get_requests_session(max_retries: int = 3):
    ...
    s.headers.update({"User-Agent": "target-druggability-pipeline"})
    return s
...
def main() -> None:
    ...
    gene_file = resolve_existing_file(
        ...
        [
            ...
            os.path.join(root, "GEO_PREP", "GSE81089", "WGCNA_GSE81089_out_signature_fix", "tables", os.path.basename(args.gene_file)),
    ...
    ppi_manifest_default = resolve_from_manifest(
        manifest_paths,
        [
            r"/ppi_network/tables/node_centrality\.csv$",
            r"/ppi_network/tables/.*node.*central.*\.csv$",
            r"/ppi_network/tables/.*central.*\.csv$",
            r"/ppi_network/tables/.*hub.*\.csv$",
        ],
    )
    ppi_file = resolve_existing_file(
        ...
        [
            ...
            os.path.join(root, "ppi_network", "tables", "node_centrality.csv"),
        ],
    )
    ...
    write_excel(outpaths["tables"] / "Target_Druggability_Results_FerrDb_Advanced.xlsx", {
