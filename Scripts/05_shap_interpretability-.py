...
def resolve_risk_path(manifest_paths: Sequence[str], cohort: str) -> Optional[str]:
    pats = [
        rf"/OUT/risks/test_{cohort}__risk__STACKING\.csv$",
        rf"/OUT/risks/test_{cohort}__risk__.*\.csv$",
...
def plot_risk_surrogate_permutation_bar(perm_df: pd.DataFrame, outdir: Path, cohort: str,
    ...
    ax.legend(handles=[mpatches.Patch(color=PAL[k], label=k) for k in ["Driver", "Suppressor", "Marker/Other"]], loc="lower right")
    plt.tight_layout()
    save_fig(fig, outdir, f"{cohort}_risk_surrogate_permutation_importance")
    plt.close(fig)
...
def plot_shap_beeswarm(shap_values: np.ndarray, X_df: pd.DataFrame, top_features: Sequence[str], outdir: Path, cohort: str) -> None:
    ...
    ax.legend(handles=[mpatches.Patch(color=PAL[k], label=k) for k in ["Driver", "Suppressor", "Marker/Other"]], loc="lower right")
    plt.tight_layout()
    save_fig(fig, outdir, f"{cohort}_surrogate_shap_beeswarm")
    plt.close(fig)
...
def plot_cox_forest(cox_df: pd.DataFrame, outdir: Path, cohort: str, top_n: int = 20) -> None:
    ...
    plt.tight_layout()
    save_fig(fig, outdir, f"{cohort}_multivariable_cox_forest")
    plt.close(fig)
...
def plot_calibration(T: np.ndarray, E: np.ndarray, risk_scores: np.ndarray, outdir: Path, cohort: str, years=(1, 3, 5)) -> Optional[pd.DataFrame]:
    ...
    plt.tight_layout()
    save_fig(fig, outdir, f"{cohort}_calibration_curves")
    plt.close(fig)
    return pd.DataFrame(rows)
...
def plot_dca(T: np.ndarray, E: np.ndarray, risk_scores: np.ndarray, outdir: Path, cohort: str, t_yr: int = 5) -> Optional[pd.DataFrame]:
    ...
    ax.legend()
    plt.tight_layout()
    save_fig(fig, outdir, f"{cohort}_decision_curve_{t_yr}yr")
    plt.close(fig)
...
def analyze_one_cohort(cohort: str, manifest_paths: Sequence[str], genes: Sequence[str], args: argparse.Namespace,
    ...
    try:
        ...
        shap_df.to_csv(outdir / f"{cohort}_surrogate_SHAP_mean_abs_importance.csv", index=False)
        ...
        shap_direction_df.to_csv(outdir / f"{cohort}_directional_SHAP_support_for_ML_genes.csv", index=False)
    ...
    comb.to_csv(outdir / f"{cohort}_combined_gene_ranking.csv", index=False)
    ...
    if not cox_df.empty:
        ...
        cox_df.to_csv(outdir / f"{cohort}_multivariate_cox_per_gene.csv", index=False)
...
def main() -> None:
    ...
    outdir.mkdir(exist_ok=True, parents=True)
    script_name = Path(__file__).name if "__file__" in globals() else "05_shap_interpretability_multicohort_external_.py"
    ...
    if not results:
        pd.DataFrame(notes).to_csv(outdir / "analysis_notes_for_manuscript.csv", index=False)
    ...
    perm_all.to_csv(outdir / "risk_surrogate_permutation_importance_all_cohorts.csv", index=False)
    shap_nonempty = [r.shap_df for r in results if not r.shap_df.empty]
    if shap_nonempty:
        shap_all = pd.concat(shap_nonempty, ignore_index=True)
        shap_all.to_csv(outdir / "surrogate_SHAP_all_cohorts.csv", index=False)
    else:
        shap_all = pd.DataFrame()

    combined_all = pd.concat([r.combined_df for r in results], ignore_index=True)
    combined_all.to_csv(outdir / "combined_gene_rankings_all_cohorts.csv", index=False)
    ...
    if direction_nonempty:
        ...
        direction_all.to_csv(outdir / "directional_SHAP_support_all_cohorts.csv", index=False)
        direction_summary = aggregate_directional_support(direction_all, gene_meta)
        direction_summary.to_csv(outdir / "directional_SHAP_support_summary.csv", index=False)
    else:
    ...
    fidelity_df.to_csv(outdir / "surrogate_fidelity_summary.csv", index=False)
    ...
    rank_table.to_csv(outdir / "cross_cohort_stability_summary.csv", index=False)
    plot_rank_heatmap(rank_table, cohorts_run, outdir, "cross_cohort_stability_heatmap",
    ...
    if not meta_importance.empty:
        meta_importance.to_csv(outdir / "meta_feature_importance_random_effects.csv", index=False)
    ...
    if not loo_df.empty:
        ...
        loo_summary.to_csv(outdir / "leave_one_cohort_out_stability.csv", index=False)
    ...
    if cox_nonempty:
        ...
        cox_all.to_csv(outdir / "multivariable_cox_all_cohorts.csv", index=False)
        meta_cox = meta_from_cox(cox_all)
        if not meta_cox.empty:
            meta_cox.to_csv(outdir / "meta_adjusted_cox_random_effects.csv", index=False)
    ...
    manuscript_top.to_csv(outdir / "manuscript_top_genes_summary.csv", index=False)
    ...
    pd.DataFrame(category_rows).to_csv(outdir / "feature_category_summary.csv", index=False)
    ...
    pd.DataFrame(files).to_csv(outdir / "output_manifest.csv", index=False)
    ...
    (outdir / "manuscript_notes.txt").write_text("\n".join(txt), encoding="utf-8")
