...
project_root <- path.expand(arg_value("--project_root", "~/test"))
...
gene_candidates <- unique(c(
  ...
  file.path(project_root, "OUT", "genes", basename(gene_file)),
  file.path(project_root, "GEO_PREP", "GSE81089", "WGCNA_GSE81089_out_signature_fix", "tables", basename(gene_file)),
...
paths <- list(
  ...
  enrich_hallmark = file.path(root, "pathway_enrichment", "tables", "table_gsea_meta_hallmark.tsv"),
  enrich_integrated = file.path(root,  "pathway_enrichment", "tables", "table_integrated_pathway_summary.tsv")
)
