...
POWER_RANGE <- 1:20
R2_TARGET <- 0.80           
FORCE_POWER <- NA_integer_  
MIN_MODULE_SIZE <- 30
MERGE_CUT_HEIGHT <- 0.25


ANALYSIS_MODE <- "both"     
EXPORT_MODULE_TOPN <- 200   
MM_MIN_ABS <- 0.30           
GS_MIN_ABS <- 0.20          
UNIV_P_CUTOFF <- 0.05        
MIN_FERRO_IN_MODULE <- 3    
MAX_MODULE_GENES_ML <- 300  
...
if (length(sel_genes) > MAX_MODULE_GENES_ML) {
  ...
  if (nrow(conservative_df) > HUB_SCORE_TOPN) {
    conservative_df_full <- conservative_df  
...
# possible_prognostic_genes_consensus = conservative ∪ discovery ∪ hub ∪ Cox-sig
possible <- sort(unique(c(
  conservative_genes,                                                            # FerrDb ∩ module
  discovery_genes,                                                               # novel candidates
  if (nrow(hub)) hub$gene else character(0),                                   
