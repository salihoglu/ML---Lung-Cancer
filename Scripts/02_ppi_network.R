...
OUT_DIR    <- file.path(SCRIPT_DIR, "ppi_network_Q1_v3_1_modern")
...
# ── 2b. MODERN PPI PLOTTING HELPERS ───────────────────────────────────────────
...
zoom_layout <- function(layout_obj, zoom = 1.20) {
  # zoom > 1 : daha yakın plan görünüm
...
string_db <- tryCatch({
  ...
  tryCatch({
    ...
    message("  STRINGdb initialised (v11.5 fallback)")
    db
  }, error = function(e2) {
