...
# v44-FIX-1: Global meta_n_jobs — dir() scoping hatası yerine global değişken
# meta_univariate_select içinde artık doğrudan parametre olarak geçiriliyor
_META_N_JOBS: int = 4  # main() tarafından args.meta_n_jobs ile güncellenir
...
# ---------------- Fixed split (v36: multi-test support) ----------------
# v36 STRATEJİSİ:
#   FIXED_TEST: Birden fazla test kohortu destekleniyor
#     GSE50081: mevcut independent test (Evre I-II, n=181)
#     GSE68465: yeni ek test (NCCTG, mixed stage, n=432) — Q1 için kritik
#   FIXED_TRAIN: GSE136961 (n=21) ve küçük kohortlar
#     min_events_per_cohort=15 ile LOCO fold dışı tutulur ama
#     train datasına katkı sağlamaya devam eder.
#   NOT: GSE68465 FIXED_TRAIN'den çıkarıldı (artık test kohortu)
FIXED_TEST = ["GSE50081", "GSE68465"]   # 2 bağımsız test kohortu
FIXED_TRAIN = [
    "GSE87340", "GSE72094", "GSE136961", "GSE157009",
    "GSE31210", "GSE30219", "TCGA-LUSC", "TCGA-LUAD"
    # GSE68465 → FIXED_TEST'e taşındı (bağımsız büyük test kohortu)
]
...
def cohort_zscore(df: pd.DataFrame, cohort: pd.Series, prefix: str = "RNA__",
    ...
    """
    v38 FIX-1: Train-stats aware normalization.
    train_stats: {cohort_name: {"mu": Series, "sd": Series}} veya None
    
    train_stats=None → kendi istatistikleri ile normalize (orijinal davranış)
    train_stats=dict → dışarıdan verilen train mu/sd uygula (leakage-free test norm)
    
    NOT: LOCO fold'da Xte küçük (n~100-300) → kendi mu/sd gürültülü ama bias yok.
    Dışarıdan stats geçirmek daha stabil ama aynı zamanda train bilgisini sızdırır.
    Bioinformatik konsensüs: LOCO-test için kendi stats kullanmak kabul edilebilir.
    External test için ise train stats kullanmak gerekir.
    """
    df2 = df.copy()
...
def extract_cohort_stats(df_normalized_source: pd.DataFrame,
    ...
    """
    v38: Train normalization istatistiklerini çıkar (external test normalizasyonu için).
    Returns {cohort: {"mu": Series, "sd": Series}} veya {"med","iqr"} if robust.
    """
...
def add_paper_like_traits(X: pd.DataFrame, ferr_driver: List[str], ferr_suppressor: List[str], module_genes: List[str]) -> pd.DataFrame:
    """
    v35: Genişletilmiş ferroptosis composite feature engineering.
    Orijinal 4 feature + 6 yeni türetilmiş feature.

    Orijinal (v34):
      FERRO__DRIVER, FERRO__SUPPRESSOR, FERRO__TOTAL, FERRO__TRAIT, WGCNA__ME1

    Yeni (v35):
      FERRO__DRIVER_PC1   : driver genlerinin PCA1 skoru (mean yerine)
      FERRO__SUPP_PC1     : suppressor genlerinin PCA1 skoru
      FERRO__NET_SCORE    : PC1_driver - PC1_suppressor (net ferroptosis aktivitesi)
      FERRO__RATIO        : log2(driver_mean / (suppressor_mean + eps)) — oran skoru
      FERRO__WGCNA_INTER  : WGCNA__ME1 × FERRO__TRAIT — modül x ferroptosis etkileşim
      FERRO__Z_COMBINED   : z-score(driver_mean) + z-score(suppressor_mean) — toplam z
    """
    df = X.copy()
    ...
    # ── v35 YENİ: Gelişmiş ferroptosis composite features ─────────────────
    eps = 1e-6

    # Feature 1 & 2: PCA1 skoru (mean yerine; ilk PC daha fazla varyansı yakalar)
    ...
    # Feature 3: Net ferroptosis aktivite skoru (PC1 bazlı)
    df["FERRO__NET_SCORE"] = df["FERRO__DRIVER_PC1"] - df["FERRO__SUPP_PC1"]

    # Feature 4: Log2 ratio (driver/suppressor) — oran bazlı sinyal
    ...
    # Feature 5: WGCNA × Ferroptosis etkileşim terimi
    # (Yüksek modül aktivitesi + yüksek ferroptosis → sinerjistik etki)
    df["FERRO__WGCNA_INTER"] = df["WGCNA__ME1"] * df["FERRO__TRAIT"]

    # Feature 6: Z-score tabanlı birleşik skor
    ...
    # v37 FIX-7: Tüm FERRO__ feature'larını z-score normalize et
    # Skala tutarsızlığı ETX/RSF'te önemli değil ama Cox/COXNET için kritik
    ...
    for fc in ferro_feature_cols:
        ...
        if np.isfinite(mu_f) and np.isfinite(sd_f) and sd_f > 1e-8:
            ...
            df[fc] = 0.0  # degenerate: constant feature → zero
...
def _parse_stage_token(tok: str) -> Optional[float]:
    """
    v44-FIX-10: pTNM formatı desteği eklendi.

    GSE68465 gibi kohortlar 'pN0pT2', 'pN1pT2' formatında stage bilgisi içeriyor.
    Eski parser bu formatı hiç tanımıyordu → 0/462 parse → stage constant → NRI/IDI atlanıyordu.

    Yeni mantık (öncelik sırası):
      1. pTNM format: pNxpTy → T skoru + 0.5 × N skoru
         T1=1, T2=2, T3=3, T4=4  (tümör boyutu — birincil prognostik)
         N0=0, N1=1, N2=2         (lenf nodu — ikincil)
         Örn: pN0pT2 → 2.0,  pN1pT2 → 2.5,  pN2pT2 → 3.0
         Bu kodlama AJCC T/N'yi sürekli bir skala olarak temsil eder;
         Cox modeli için anlamlı ve ordinal sıralamayı korur.
      2. AJCC roman numeral: I/II/III/IV → 1/2/3/4
      3. Arabic numeral: 1/2/3/4
      4. Tanınamıyorsa: None
    """
    if tok is None:
        return None
...
def extract_stage_from_tcga_clinical(clinical_path: Path, sample_ids: pd.Index) -> pd.DataFrame:
    ...
    stage_col = None
    if cand:
        # prefer ajcc_pathologic_stage if exists
        for c in df.columns:
...
def extract_stage_from_geo_pheno(pheno_path: Path, sample_ids: pd.Index) -> pd.DataFrame:
    """
    v44-FIX-10: _parse_stage_token artık pTNM (pN0pT2) formatını da parse ediyor.
    stage_col seçim önceliği güncellendi: disease_stage / pathologic_stage / stage sırası.
    stage_map tipi int → float (TNM skoru ondalıklı olabilir: pN1pT2 → 2.5).
    """
    ...
    # Öncelik sırası: pt.stage+pn.stage (GSE30219 pTNM) > disease_stage > patholog* > ajcc* > genel "stage"
    stage_col  = None
    stage_map: Dict[str, float] = {}  # initialized here — used by both paths below
    ...
    # Eğer ayrı T/N kolonları varsa → birleştir (pTNM format — GSE30219 tipi)
    ...
    # Generic fallback: tek kolon (disease_stage, ajcc_pathologic_stage vb.)
    if not stage_map:
    ...
    if _parsed == 0 and stage_col is not None:
        print(f"  [v44 WARN] extract_stage_from_geo_pheno: stage_col='{stage_col}' "
              f"parse edilemedi (0/{len(df)} satır). Örnek değer: "
              f"'{df[stage_col].dropna().iloc[0] if df[stage_col].notna().any() else 'ALL NaN'}'")
    else:
        print(f"  [v44] Stage parsed: {_parsed}/{len(sample_ids)} samples "
...
def load_geo(cohort_name: str, cohort_dir: Path, add_stage=True, gene_subset=None,
    ...
    """
    v45-1: hist_filter desteği eklendi.
    hist_filter: {"col": "histology", "values": ["ADC"]} formatında dict.
    GSE30219 gibi mixed-histoloji kohortları için ADC-only filtre uygulanır.
    Filtre pheno_expanded.csv'den okunur; sample ID eşleşmesi GSM prefix ile yapılır.
    """
        ...
        if pheno_path.exists():
            try:
                ...
                if _col_match:
                    ...
                    t2  = t2.loc[X2.index]
                    e2  = e2.loc[X2.index]
                    print(f"  [v45] {cohort_name} hist_filter '{_hcol}'={_hvals}: "
                          f"{n_before} → {len(X2)} samples "
                          f"({int(e2.sum())} events)")
                else:
                    print(f"  [v45 WARN] {cohort_name}: hist_filter col '{_hcol}' not found "
            ...
            print(f"  [v45 WARN] {cohort_name}: pheno_expanded not found for hist_filter — skipped")
...
def load_wgcna_genes_from_file(path: Path, prefer_branch: Optional[str] = None) -> List[str]:
    """
    v36: WGCNA gen dosyasını okur.
    - wgcna_genes_for_ML.csv'de 'branch' kolonu varsa hem conservative hem
      discovery genleri var; prefer_branch='conservative' ile sadece
      FerrDb-onaylı genleri alabilirsin (varsayılan: tümünü al)
    - conservative_ferroptosis_genes_GS_MM.tsv'de hub_score kolonu var;
      hub_score'a göre sıralı gelir, zaten seçici
    """
...
def _pick_module_from_trait_stats(trait_stats: pd.DataFrame) -> Optional[str]:
    """
    Heuristik: 'ferro' içeren trait satırlarından en yüksek pozitif korelasyonlu modülü seç.
    Dosya formatı değişken olabilir; bu yüzden esnek arıyoruz.
    """
...
def load_wgcna_from_path(wgcna_path: Optional[Path]) -> Tuple[List[str], str]:
    ...
    base = wgcna_path
    tables = base / "tables"
    # ── v36: Dosya öncelik sırası v13 WGCNA çıktılarına göre güncellendi ──────
    # Öncelik mantığı (en seçiciden en genişe):
    #   1. wgcna_genes_for_ML.csv          — v13 conservative∪discovery (hub-score sıralı, ML için optimize)
    #   2. conservative_ferroptosis_genes_GS_MM.tsv — sadece FerrDb∩module (en seçici, biyolojik filtreli)
    #   3. module_hub_genes_MM_GS.tsv      — yüksek MM+GS hub genler
    #   4. module_genes_GS_MM.tsv          — tüm modül genleri (GS/MM skorsuz)
    #   5. possible_prognostic_genes_consensus.csv — tüm adaylar (en geniş, en az seçici)
    candidates = [
        tables / "wgcna_genes_for_ML.csv",                      # v13 ML-optimized (en iyi)
        tables / "conservative_ferroptosis_genes_GS_MM.tsv",    # v13 FerrDb∩module hub-scored
        tables / "module_hub_genes_MM_GS.tsv",                  # klasik hub genler
        tables / "module_genes_GS_MM.tsv",                      # tüm modül genleri
        tables / "possible_prognostic_genes_consensus.csv",      # tüm adaylar (fallback)
    ]
    for p in candidates:
        if p.exists():
    ...
    # try gene_modules.tsv (+ module_trait_stats.tsv)
    ...
    if gm.exists():
        ...
        if mod_col is None:
            # if only 2 cols assume second is module
            if len(df.columns) >= 2:
...
def _detect_time_unit(y_time: np.ndarray) -> str:
    """Survival süresinin gün mü yıl mı olduğunu tahmin et."""
...
def cindex(y_time: np.ndarray, y_event: np.ndarray, risk: np.ndarray,
    ...
    """
    v37 FIX-1: Uno's IPCW C-index varsayılan.
    Yüksek sansürleme oranında Harrell C-index tutarsız sonuç üretir.
    Uno et al. (2011) Stat Med: IPCW-weighted concordance daha robust.

    use_ipcw=True  → Uno's C (sksurv.metrics.concordance_index_ipcw)
    use_ipcw=False → Harrell C (Fallback)
    """
    ...
    if use_ipcw and SKSURV_AVAILABLE:
        try:
            ...
            # tau = 75th percentile of observed event times (Uno 2011 recommendation)
...
def _eval_one_gene(args_tuple):
    """v36 FIX-6: Meta-univariate paralel hesaplama için tek gen değerlendirmesi."""
    ...
    # v37 FIX-5: Per-cohort concordance score eklendi
    # Cox z/p tek başına yeterli değil — küçük SE ile güçlü görünen ama
    # gerçekte düşük concordance olan genler elenebilir
    concordance_scores = []
...
def meta_univariate_select(Xtr2, ttr, etr, cohort_series, candidate_rna_cols, clin_cols,
    ...
    rows=[]
    candidate_rna_cols = candidate_rna_cols[:600]   # v40: 1200→600 (3.2x hız, kalite farkı minimal)

    # v36 FIX-6: ThreadPoolExecutor ile paralel gen değerlendirmesi
    # Tek-thread fallback: meta_n_jobs=1 veya ThreadPoolExecutor başarısız olursa
    import concurrent.futures as _cf

    # Ortak array'ler hazırla (thread-safe read-only paylaşım)
    ...
    # v44-FIX-1: n_jobs artık parametre olarak geçiriliyor (dir() hack kaldırıldı)
    ...
    if n_jobs_use > 1:
        try:
            ...
            # Paralel başarısız → seri fallback
    ...
    # v37 FIX-5: Concordance score birincil, p_meta ikincil sıralama
    ...
    # ── v35: Adaptif eşik — çok az gen seçildiyse otomatik gevşet ────────────
    # Eğer seçilen gen sayısı hedefin %30'undan azsa, eşikleri gevşet ve tekrarla.
    ...
    if len(out) < target_min and (float(min_frac_cohorts) > 0.30 or int(min_events_per_cohort) > 6):
        ...
        if rows2:
            ...
            print(f"  [v35 adaptive] Relaxed threshold (frac≥{relaxed_frac}, ev≥{relaxed_events}): "
                  f"+{len(out2)} genes, total={len(out)}")

    # --- v31: Benjamini-Hochberg FDR düzeltmesi ---
    if len(out) > 1:
...
# ---------------- Fixed-year endpoint metrics (1/2/3/5-year) ----------------
...
# ---------------- Paper-ready evaluation (KM, IPCW-AUC, Brier/IBS, Calibration, DCA, Bootstrap) ----------------
def time_dependent_auc_ipcw(y_train_time, y_train_event, y_test_time, y_test_event, risk, years_list=(1,2,3,5)) -> pd.DataFrame:
    ...
    try:
        ...
        # v37 FIX-2: Survival time birimi otomatik tespit
        # Medyan survival > 50 → gün cinsinden → yıla çevir
        # Medyan survival < 50 → zaten yıl cinsinden
        ...
        # Remove time points beyond max observed time (causes IPCW to fail)
...
# ============================================================
# v31 YENİ FONKSİYONLAR
# ============================================================

def permutation_cindex_pvalue(y_time, y_event, risk, n_perm=1000, seed=13):
    """
    Permütasyon testi: gözlemlenen C-index rastlantısal mı?
    H0: risk skoru survival ile ilişkisiz (risk permüte edilir).
    Döndürür: {"observed_cindex", "perm_mean", "perm_std", "p_value", "n_perm"}
    """
...
def continuous_nri_idi(y_time: np.ndarray, y_event: np.ndarray,
                       ...
                       years: int = 3) -> dict:
    """
    v37 FIX-9: Continuous NRI and IDI (Pencina et al. 2008 Stat Med).
    Baseline: stage-only or simple Cox model
    New: risk_score + baseline

    NRI (continuous): sum of net upward reclassification in events +
                      net downward reclassification in non-events
    IDI: difference in discrimination slopes (mean predicted - mean baseline)

    Q1 zorunluluğu: Risk prediction model improvement must be quantified.
    """
    result = {
    ...
    try:
        ...
        if n_ev > 0 and n_nev > 0:
            ...
            # v44-FIX-4: Bootstrap SE for NRI (Pencina 2008 uyumlu, IDI ile tutarlı)
            # Önceki: Analitik DeLong-like SE (IDI ile tutarsız)
            # Yeni: 500 bootstrap replikası → 95%CI ve iki-taraflı p
        ...
        # IDI (Pencina 2008): difference in discrimination slopes
        # IS_new - IS_base where IS = mean(predicted|event) - mean(predicted|non-event)
...
def cox_ph_assumption_test(y_time: np.ndarray, y_event: np.ndarray,
    ...
    """
    v37 FIX-3: Cox proportional hazards assumption test.
    Schoenfeld residual korelasyonu + global chi-square testi.
    Q1 zorunluluğu: PH varsayımı test edilmeden Cox raporu yetersiz sayılır.
...
def expected_observed_ratio(y_time: np.ndarray, y_event: np.ndarray,
                             ...
                             years: int = 3) -> dict:
    """
    v37 FIX-4: Expected/Observed (E/O) ratio for calibration.
    Q1 zorunluluğu: Sadece kalibrasyon eğrisi yeterli değil; E/O ratio
    numerik kalibrasyon kanıtı olarak raporlanmalı.

    E = Toplam predicted event (Cox survival olasılığından)
    O = Gözlemlenen gerçek event (Kaplan-Meier tahmininden)
    E/O = 1 → mükemmel kalibrasyon
    E/O > 1 → model aşırı tahmin (over-prediction)
    E/O < 1 → model eksik tahmin (under-prediction)
    """
    result = {
    ...
    try:
        ...
        # Observed: Kaplan-Meier event probability at horizon
        ...
        # v44-FIX-3: Cox-baseline predicted survival (bireysel predicted prob)
        # Önceki: KM risk-grup proxy (sadece 2 grup, kaba tahmin)
        # Yeni: Cox-on-risk ile her hastanın bireysel S_pred(t) hesaplanıyor
        surv_probs = _cox_on_risk_survival_probs(
        ...
        if surv_probs is not None and surv_probs.shape[0] == len(r):
            ...
            # Fallback: KM risk-group proxy (Methods'ta belirtilecek)
...
def plot_calibration(cal_df: "pd.DataFrame", title: str, out_png: "Path"):
    """v31: Calibration curve figürü kaydeder."""
...
def _km_group_from_risk(t: np.ndarray, e: np.ndarray, r: np.ndarray) -> np.ndarray:
    """
    Direction-safe KM grouping:
    grp=1 => 'High risk' and, when estimable, corresponds to worse prognosis.
    """
...
# ============================================================
# v45 YENİ FONKSİYONLAR: RMST + Landmark (PH ihlali için)
# ============================================================

def compute_rmst(y_time: np.ndarray, y_event: np.ndarray, risk: np.ndarray,
                 tau_years: float = 3.0, n_boot: int = 500, seed: int = 13) -> dict:
    """
    v45-2: Restricted Mean Survival Time (RMST) analizi.

    PH varsayımı gerektirmez — GSE68465 gibi PH ihlali olan kohortlar için
    C-index'e alternatif/tamamlayıcı bir survival summary metric.

    RMST(tau) = E[min(T, tau)] = integral_0^tau S(t) dt
    RMST farkı (high risk vs low risk): delta_RMST = RMST_low - RMST_high
    delta_RMST > 0 → yüksek riskli grup daha az süre yaşıyor (doğru yön)

    Döndürür: delta_rmst, rmst_low, rmst_high, p_value (log-rank proxy), CI
    """
    result = {
...
def landmark_analysis(y_time: np.ndarray, y_event: np.ndarray, risk: np.ndarray,
    ...
    """
    v45-3: Landmark analizi — PH ihlalini zamanla değişen etki olarak belgeler.

    Her landmark t*'de:
      - t* öncesi yaşayan hastalar seçilir (conditional population)
      - Bu alt grupta t* → t* + follow_years penceresi için HR ve log-rank hesaplanır
      - HR değeri zamana göre değişiyorsa (erken/geç etki farkı) PH ihlali doğrulanır
      - Manuscript'te: "time-varying effect" tablosu olarak raporlanabilir

    Döndürür: DataFrame(landmark_year, n_at_risk, events_in_window, hr, p_logrank)
    """
    if not LIFELINES_AVAILABLE:
        return pd.DataFrame()
    rows = []
...
def enforce_ferroptosis_presence(selected_features: List[str], ferr_genes: List[str],
    ...
    """
    v44-FIX-6: final_gene_cap sonrası RNA gen sayısı garantili olarak kırpılıyor.
    Önceki: cap aşılabiliyor (forced genler cap'i görmezden geliyordu).
    Yeni: RNA genleri eklendikten sonra cap uygulanıyor, non-RNA (CLIN__, FERRO__) korunuyor.
    """
    ...
    if final_gene_cap is not None:
        ...
        if add:
            print(f"  [v44] enforce_ferroptosis: +{len(add)} forced ferro genes, "
...
def fit_predict_rsf(Xtr, ttr, etr, Xte, n_estimators, min_leaf, max_features, max_depth, seed):
    # v35: min_samples_split düşürüldü (max(2, min_leaf) → daha derin ağaçlar)
    # v33: max(6, 2*min_leaf) → v35: max(2, min_leaf) — leaf=2/3 ile daha derin
    ytr = _build_sksurv_y(ttr, etr)
    min_split = max(2, int(min_leaf))   # v35: daha agresif bölme (v33: max(6,2*min_leaf))
    model = RandomSurvivalForest(
        ...
        n_jobs=1, random_state=int(seed)  # v36 FIX-5
    )
    model.fit(Xtr.values, ytr)
    try:
        return np.asarray(model.predict(Xte.values), dtype=float)
...
def fit_predict_extratrees(Xtr, ttr, etr, Xte, n_estimators, min_leaf, max_features, max_depth, seed):
    # v36 FIX-2: min_samples_split dinamik (leaf ile uyumlu); eskisi sabit 10 idi
    # leaf=2,3 için sabit 10 anlamsız — ağaç hiç büyümüyordu
    ytr = _build_sksurv_y(ttr, etr)
    dyn_split = max(2, int(min_leaf))   # leaf=2→split=2, leaf=5→split=5, leaf=20→split=20
    model = ExtraSurvivalTrees(
        n_estimators=int(n_estimators), min_samples_split=dyn_split,
        min_samples_leaf=int(min_leaf),
        max_features=max_features, max_depth=None if max_depth is None else int(max_depth),
        n_jobs=1, random_state=int(seed)  # v36 FIX-5: int() cast
    )
    model.fit(Xtr.values, ytr)
    try:
        return np.asarray(model.predict(Xte.values), dtype=float)
...
def build_algorithm_configs(light=True, prior_leaderboard_csv=None, prior_min_cindex=0.55,
                             ...
                             etx_ne_values=None):
    """
    v41 — 150 config, 9 family eşit (16-17 her biri), data-driven parametreler.

    Leaderboard analizi (586 config, v40 çıktısı):
    ┌──────────────┬──────┬────────┬───────────────────────────────────────────┐
    │ Family       │  n   │  max C │ Optimal parametreler                       │
    ├──────────────┼──────┼────────┼───────────────────────────────────────────┤
    │ EXTRATREES   │ 174  │ 0.7231 │ mf=0.65, leaf=8, ne=300-500, md=8        │
    │ RSF          │ 144  │ 0.7148 │ mf=0.60, leaf=20, ne=400-500, md=8-None  │
    │ GBM          │  72  │ 0.7163 │ lr=0.02, ne=100-200, md=2-3, sub=0.80    │
    │ COXNET       │  72  │ 0.7075 │ l1=0.1-0.5, amin=0.10                    │
    │ CWGBM        │  48  │ 0.6886 │ lr=0.05-0.10, dr=0.0 (dr>0 → risk=-1)   │
    │ COXPH        │  10  │ 0.6914 │ pen=0.05-0.50                             │
    │ SKSCOXPH     │   9  │ 0.6475 │ alpha=1.0-5.0                             │
    │ FASTSVM      │  21  │ 0.5715 │ rr=1.0 ZORUNLU (rr<1.0 → risk=-1)       │
    │ SURVTREE     │  36  │ 0.6212 │ leaf=12-30, mf=0.60, md=6-None           │
    └──────────────┴──────┴────────┴───────────────────────────────────────────┘

    risk_sign=-1 kökeni:
      CWGBM: dropout_rate > 0 → model ters yön öğreniyor → dr=0.0 sabitlendi
      FASTSVM: rank_ratio < 1.0 → C-index < 0.5 → rr=1.0 sabitlendi

    Dağılım: 17 config × 6 family + 16 config × 3 family = 150 toplam
    (ETX, RSF, GBM, COXNET, CWGBM, COXPH = 17; SURVTREE, SKSCOXPH, FASTSVM = 16)
    """
    algos = []
    ...
    # Data: lr=0.02 dominant (mean 0.711!), ne=100-200, md=2-3, sub=0.80
    # risk=-1: YOK → parametreler güvenli
    algos_gbm = []
    ...
    # ── 4. CoxNet (ElasticNet-Cox) — 17 config ────────────────────────────────
    # Data: amin=0.10 dominant (mean 0.689), l1=0.1-0.5 aralığı
    # L1=1.0 (LASSO) da denenecek → feature selection
    algos_coxnet = []
    ...
    # ── 5. ComponentWise GBM — 17 config ──────────────────────────────────────
    # Data: dr=0.0 ZORUNLU (dr>0 → risk=-1 guaranteed)
    #       lr=0.10 best, lr=0.05 runner-up, ne büyük iyi
    # risk=-1 fix: dropout_rate=0.0 sabitlendi
    algos_cwgbm = []
        ...
        for lr in [0.10, 0.05, 0.03]:
            algos_cwgbm.append((f"CWGBM_ne{ne}_lr{lr}_dr0.0", "CWGBM",
                                 {"n_estimators": ne, "learning_rate": lr,
                                  "dropout_rate": 0.0}))  # dr=0.0 ZORUNLU
    # lr=0.01 ek 2 config (17'ye tamamla)
    ...
    # ── 6. CoxPH — 17 config ──────────────────────────────────────────────────
    # Data: pen=0.50 best (0.691), pen=0.10 runner-up (0.690)
    # Geniş sweep: log-scale penalizer
    ...
    # ── 7. SKSurvival CoxPH — 16 config ───────────────────────────────────────
    # Data: alpha=5.0 best (0.648), alpha=2.0 runner-up
    # Geniş sweep: log-scale alpha
    ...
    # ── 8. FastSurvivalSVM — 16 config ────────────────────────────────────────
    # Data: rr=1.0 ZORUNLU (rr=0.0 ve rr=0.5 → risk=-1 guaranteed)
    #       rr=1.0 ile C-index=0.5715 sabit → alpha sweep pek fark yaratmıyor
    #       Ama makale için family karşılaştırmasında gerekli
    # risk=-1 fix: rank_ratio=1.0 sabitlendi
    ...
    # ── 9. SurvivalTree — 16 config ────────────────────────────────────────────
    # Data: leaf=12 + mf=0.60 + md=None best (0.621)
    #       leaf büyük olunca daha stable
    algos_st = []
...
def select_diverse_topk(leaderboard: pd.DataFrame, k: int, min_param_distance: float = 0.28, max_per_family: int = 4) -> pd.DataFrame:
    ...
    # keep ranking order from leaderboard for readability
...
def compute_sample_weights(cohort_series: pd.Series,
    ...
    if event_series is not None and len(event_series) == len(cohort_series):
        ...
        for coh in counts:
            ...
            event_rates[coh] = max(er, 0.05)  # floor 5% to avoid div/0
        w = cohort_series.map(
    ...
    w = np.clip(w, 0, np.percentile(w, 95))  # clip extreme weights
    mean_w = np.mean(w)
...
def optimize_ensemble_weights(train_risks, t, e, n_iter, seed):
    """
    v36 FIX-1: Nelder-Mead + Dirichlet warm-start.

    Strateji:
      1. Dirichlet örneklemi ile en iyi başlangıç noktasını bul
         (n_warmstart = min(300, n_iter//5) iterasyon)
      2. Nelder-Mead ile softmax parametrizasyonu üzerinde lokal optimizasyon
         (ağırlıklar her zaman ≥0, toplamı=1 garantisi)
      3. En iyi sonucu döndür

    Avantaj: 3000 Dirichlet yerine 300 warmstart + ~200 Nelder-Mead iterasyon
             → ~10x daha az hesaplama, daha iyi global optimum
    """
    ...
    # Softmax logit başlangıç noktası (warm-start ağırlığından)
    w0_safe = np.clip(best_w_ws, 1e-6, None)
    logits0  = np.log(w0_safe)
    logits0  -= logits0.mean()

    # ── Aşama 2: Nelder-Mead optimizasyonu ───────────────────────────────
    ...
    try:
        res = sp_minimize(
            ...
            options={
                ...
                "xatol": 1e-5,
                "fatol": 1e-5,
                "adaptive": True,      # boyut-bağımsız simplex adımları
            }
        )
        ...
        pass  # Nelder-Mead başarısız → warm-start sonucu kullan

    # ── Aşama 3: Kalan bütçeyle ek Dirichlet (opsiyonel) ─────────────────
...
def softmax_weighted_ensemble(risks: List[np.ndarray], scores: np.ndarray, temperature: float = 0.1) -> np.ndarray:
    """v32: Softmax-weighted rank ensemble. Temperature controls sharpness."""
...
def trimmed_rank_ensemble(risks: List[np.ndarray], trim_frac: float = 0.10) -> np.ndarray:
    """
    v36 FIX-7: Trimmed mean ensemble.
    Her örnek için tüm modellerin rank-normalize edilmiş tahminlerini alır,
    en düşük ve en yüksek trim_frac dilimini çıkarır, kalanların ortalamasını alır.
    Aykırı modellerin etkisini baskılar.
    """
    if not risks:
        return np.array([])
    ...
    Z_sorted = np.sort(Z, axis=0)                      # sırala (küçükten büyüğe)
    Z_trimmed = Z_sorted[trim_k: n_models - trim_k, :] # uçları at
    return np.nanmean(Z_trimmed, axis=0)


def borda_count_ensemble(risks: List[np.ndarray]) -> np.ndarray:
    """v32: Borda count — sum of ranks across all models."""
    n = len(risks[0])
...
def stacking_ensemble_cox(oof_risks: List[np.ndarray], oof_t: np.ndarray, oof_e: np.ndarray,
    ...
    """
    v35: Çok seviyeli stacking ensemble.
    v44-FIX-2: OOF sızıntısı düzeltildi.
      - Meta-learner seçimi: OOF üzerinde fit + OOF predict → en iyi learner seçilir
      - Test tahmini: seçilen learner ile test_risks üzerinde predict (OOF değil)
      Önceki hata: test_risks = oof_risks geçiriliyordu → sızıntı
    """
    if not LIFELINES_AVAILABLE or len(oof_risks) < 2:
        return None
    if len(test_risks) != len(oof_risks):
        print(f"  [WARN v44] stacking: oof_risks({len(oof_risks)}) != test_risks({len(test_risks)}), skip")
        return None
    ...
    print(f"  [v35 stacking] Best meta-learner OOF C-index={best_ci:.4f}")
    return best_pred
...
def bagged_meta_univariate_select(Xtr2, ttr, etr, cohort_series, candidate_rna_cols, clin_cols,
                                   ...
                                   n_jobs: int = 1):
    """
    v32: Bootstrap-bagged meta-univariate gene selection.
    v44-FIX-1: n_jobs parametresi eklendi; meta_univariate_select'e geçiriliyor.
    """
    rng = np.random.default_rng(seed)
    cohorts = cohort_series.unique().tolist()
    ...
    # ── v35: Stability filter — bag_frac >= stability_min_freq olan genler öncelik ──
    # Bu flag bagged_meta_univariate_select'ten dışarıya geçirilemiyor,
    # bu yüzden bag_frac >= 0.60 olan genler ilk sıralarda yerleştirilir
    # (sıralama: yüksek bag_frac → yüksek |z_meta|)
    stable_mask = out["bag_frac"] >= 0.60
    if stable_mask.sum() >= max(3, int(top_genes * 0.20)):
        # Stabil genleri öne al, geri kalanlar son sırada
        ...
        print(f"  [v35 stability] {stable_mask.sum()} stable genes (bag_frac≥0.60) prioritized")
...
def main():
    ...
    ap.add_argument("--top_rna_var_prefilter", type=int, default=1800,
                    help="Varyans prefilter: top N gen (v35 varsayılan: 1800, v34: 2400)")
    ap.add_argument("--meta_top_genes", type=int, default=60,
                    help="Meta-univariate gen seçimi: top N gen (v35 varsayılan: 60, v34: 100/120)")
    ...
    ap.add_argument("--min_events_per_cohort", type=int, default=15,
                    help="v36: 12→15. GSE136961(n=21) gibi küçük kohortlar LOCO fold dışı kalır")
    ap.add_argument("--min_loco_events", type=int, default=20,
                    help="v36 YENİ: LOCO fold olarak kullanılmak için minimum event sayısı. "
                         "Bu şartı sağlamayan kohortlar sadece train pool'a eklenir, fold olmaz.")
    ap.add_argument("--min_frac_cohorts", type=float, default=0.50,
                    help="v36: 0.45→0.50. Daha tutarlı gen seçimi (daha fazla train ile tolere edilebilir)")
    ...
    # Subtype stratification / filtering
    # joint: tüm kohortlar (varsayılan)
    # luad_only: TCGA-LUSC (ve LUSC olarak işaretlenen kohortlar) TRAIN'den çıkarılır
    # lusc_only: sadece LUSC kohortlarıyla çalış (en az 2 LUSC kohortu yoksa hata verir)
    ...
    ap.add_argument("--geo_subtype_map", default="", help="Opsiyonel JSON veya dosya yolu. Örn: {'GSE12345':'LUAD', 'GSE67890':'LUSC'}. Boşsa GEO varsayılanı LUAD.")
    ...
    # v35: meta_top_genes varsayılanı düşürüldü (120→60)
    # Not: Bu sadece varsayılanı değiştirir; --meta_top_genes ile override edilebilir
    ...
    # v43: stabilize final RNA signature before full-train fitting
    ...
    # v44-FIX-5: Dinamik test kohortu seçimi
    ...
    # v45-1: Histoloji filtresi — mixed-histoloji kohortlar için
    ...
    # v44-FIX-1: Global meta_n_jobs güncelleniyor
    global _META_N_JOBS
    ...
    # v31: run config reproducibility logu
    try:
    ...
    # FerrDb: driver/suppressor ayrı + birleşik
    ...
    # Gene universe (yüklemede subset kullanmak için)
    ...
    # LUSC yalnızca joint modda yüklenir; luad_only modda I/O atlanır
    _load_lusc = (args.subtype_mode != "luad_only")
    # v45-1: Histoloji filtresi — JSON parse
    ...
    # fixed split — v44-FIX-5: --test_cohorts ile dinamik override
    ...
    # subtype-aware train filtering (optional)
    subtype_map = {}
    ...
    # LOCO folds
    # v36: min_loco_events kontrolü — küçük kohortlar (GSE136961 gibi)
    # fold olarak kullanılmaz ama train pool'a katkı sağlar
    min_loco_ev = getattr(args, "min_loco_events", 20)
    folds={}
    train_set = [c for c in cohorts if c.cohort in train]

    # Hangi kohortların yeterli event'i var? (LOCO fold olabilir)
    loco_eligible = []
    loco_poolonly  = []   # fold dışı, sadece train pool'a katkı
    for c in train_set:
    ...
    for holdout in loco_eligible:   # sadece eligible kohortlar fold olur
        ...
        # v38 FIX-1: LOCO test normalization kendi stats ile yapılıyor
        # (küçük test seti için kendi stats gürültülü ama bias-free)
        Xte2 = cohort_zscore(te.X, pd.Series([holdout]*len(te.X), index=te.X.index), robust=args.robust_scaler)

        # optional paper-like traits (FERRO__*, WGCNA__ME1)
        ...
        # WGCNA filter (paper: WGCNA module genes -> downstream survival selection)
        ...
        if args.use_meta_univariate and LIFELINES_AVAILABLE:
            if args.use_bagged_gene_select:
                # v32: bootstrap-bagged gene selection
        ...
        if feats:
            folds[holdout] = {
                ...
                "sw": compute_sample_weights(coh_series, etr) if args.batch_weighting else None  # v37 FIX-6
            }
    ...
    # algo grid — v32: family_budget, min_family_slots, etx_ne_values
    etx_ne_list = None
    ...
    # v39 FIX: Round-robin interleaving — her family sırayla çalışır
    # Önceki sorun: ETX önce → TIME_GUARD ile GBM/COXNET hiç çalışmıyordu
    # Çözüm: familylere göre grupla, round-robin ile karıştır
    from collections import defaultdict
    family_queues = defaultdict(list)
    for algo in all_algos:
        family_queues[algo[1]].append(algo)

    # Priority sırası: önce en az temsil edilen familyler
    # (diverse_topk zaten çeşitlilik sağlıyor ama grid sırası da önemli)
    priority_order = ['GBM','COXNET','CWGBM','COXPH','SKSCOXPH','FASTSVM',
                      'SURVTREE','RSF','EXTRATREES']
    # Önce priority listesindekiler, sonra kalanlar
    ...
    # internal LOCO
    # v39 FIX: Checkpoint'ten devam — mevcut leaderboard'da olan configleri atla
    ...
    rows = []
    run_logs = []
    # v38 FIX: n_fold_total = gerçek fold sayısı (folds dict boyutu)
    ...
    for k, (algo_name, family, params) in enumerate(algo_grid, start=1):
        ...
        # v39 FIX: Checkpoint'te varsa atla
        ...
        # v40 FIX-1: Paralel fold değerlendirmesi
        # n_parallel_folds > 1 → ProcessPoolExecutor ile ~2.5x hız
        ...
        # v39 FIX: Her 25 config'de checkpoint kaydet (TIME_GUARD'dan kurtarmak için)
        if k % 25 == 0 and rows:
    ...
    # v39 FIX: Checkpoint ile birleştir (TIME_GUARD ile kesilmiş run'lar için)
    ...
    # v35/v44: Çok seviyeli stacking ensemble (varsayılan AÇIK)
    # v44-FIX-2: Meta-learner seçimi için OOF kullanılıyor (doğru),
    #            test tahmini için ise external test riskleri geçirilecek (aşağıda)
    ...
    if args.use_stacking and len(filt_risks) >= 2 and LIFELINES_AVAILABLE:
        ...
        # OOF selection check: test_risks=filt_risks → meta-learner seçimi (leak-free: sadece OOF'ta fit+predict)
    ...
    Xtr2 = cohort_zscore(Xtr, coh_series, robust=args.robust_scaler)
    # v38: Extract train normalization stats for external test use
    ...
    if args.use_meta_univariate and LIFELINES_AVAILABLE:
        ...
        # Robust fallback: ensure non-empty prognostic gene tables for reporting/publication
        if meta_full is None or meta_full.empty:
            meta_full = fallback_univariate_gene_select(
                Xtr2, ttr, etr, cand, top_genes=max(int(args.meta_top_genes), 40)
            )
            # force ferroptosis-related representation (FerrDb/WGCNA) when possible
        ...
        # Final hard guard: never write empty files
        if meta_full is None or meta_full.empty:
        ...
        # no meta-univariate: still populate publication-friendly files
        sel_rna = cand[:args.meta_top_genes]
        meta_full = pd.DataFrame({
            "feature": sel_rna,
    ...
    # v43: LOCO-stable final RNA signature kilidi
    locked_rna = []
    ...
    # v31/v44: nihai feature setinde minimum ferroptosis geni garantisi
    # v44-FIX-6: final_gene_cap enforce_ferroptosis içinde de uygulanıyor
    ...
    # v36 FIX-8: WGCNA gen takibi — hangi WGCNA genleri seçildi?
    if wgcna_genes:
    ...
    for r in topk.itertuples(index=False):
        ...
        for tc in test:
            ...
            # v38 FIX-1: External test set → train stats ile normalize (leakage-free + stable)
            ...
            # v37 FIX-8: Bootstrap CI for external C-index
            ...
            # Brier/IBS via Cox-on-risk (uses in-sample train risk)
            ...
            # v31: Permütasyon C-index p-değeri
            try:
            ...
            # v31: Çok değişkenli Cox doğrulaması
            ...
            # v45-2: RMST analizi — PH ihlali olan kohortlar için birincil robust metric
            # GSE68465'te PH ihlali var → C-index'e ek olarak RMST(3yr) ve RMST(5yr) raporla
            try:
                for _rmst_yr in [3, 5]:
            ...
            # v45-3: Landmark analizi — PH ihlali varsa zamanla değişen etkiyi belgele
            # Her model için çalıştırmak yavaşlatır; sadece PH ihlali olan kohortlarda yap
            ...
            # v37 FIX-9 / v44-FIX-9: NRI/IDI — çift strateji
            # Strateji A (tercih): stage-only Cox baseline (stage bilgisi varsa)
            # Strateji B (fallback): null model baseline = rank-avg risk (stage yoksa/constant)
            #   GSE68465 gibi stage bilgisi olmayan kohortlarda NRI/IDI yine de hesaplanır,
            #   "null_model" olarak etiketlenerek kaydedilir ve manuscript'te dipnot eklenir.
            try:
                if not LIFELINES_AVAILABLE:
                    raise RuntimeError("lifelines missing")
                ...
                if _stage_usable:
                    # ── Strateji A: stage-only Cox (tercih) ──────────────────────
                    ...
                    # ── Strateji B: null model baseline (stage yok / constant) ───
                    # Null model = constant risk (tüm hastalar için median)
                    # NRI/IDI burada "modelimiz null'dan ne kadar iyi?"yi ölçer
                    # Manuscript'te: "NRI vs. null model (no covariates), GSE68465 lacked stage info"
    ...
    for tc in test:
        ...
        # v32: Stacking ensemble (Cox meta-learner on test risks)
        ci_stack = float("nan")
        ...
        # v35: ETX + RSF cross-family hybrid — OOF-optimize edilmiş ağırlıklar
        ...
        try:
            ...
            if etx_risks_te and rsf_risks_te:
                ...
                # v33 sabit ağırlık (0.6/0.4) — baseline
                ...
                # v35 YENİ 1: OOF-optimize edilmiş ağırlık (ETX vs RSF)
                # OOF ETX ve RSF risk vektörlerini alıp optimize_ensemble_weights ile
                # en iyi ağırlığı bul, sonra test'e uygula
                try:
                    etx_oof_risks = []
                    rsf_oof_risks = []
                    for nm in oof_names:
                ...
                # v35 YENİ 2: ETX + RSF + Stacking üçlü ensemble
                if stacking_oof_pred is not None and len(ci_stack_arr := []) == 0:
                    try:
                        # stacking_oof_pred zaten var; test için stacking tahminini al
                        # (stacking test prediction zaten ci_stack olarak hesaplandı)
                        pass
                    except Exception:
                        pass
                ...
                # v35 YENİ 3: GBM dahil üçlü hybrid (ETX + RSF + GBM)
                if gbm_risks_te:
        ...
        # v36 FIX-7: Trimmed rank ensemble
    ...
    with pd.ExcelWriter(outdir/"summary_best_tables.xlsx") as w:
        ...
        # v35: Best ensemble per method — hangi ensemble her test'te en iyi?
        if not ens_df.empty:
            try:
                ...
                pass
        # v45: RMST summary (PH ihlali için birincil robust metric)
        try:
        ...
        # v45: Landmark summary (time-varying HR)
        try:
            ...
            print(f"[WARN v45] Landmark sheet failed: {_le}")
        # v35: v35 features summary
        v35_summary = pd.DataFrame([{
            "improvement": "Agresif gen seçimi",
            "param": "meta_top_genes=60, prefilter=1800",
            "expected_delta_cindex": "+0.005..+0.015",
            "status": "aktif"
        }, {
            "improvement": "Ferro composite features",
            "param": "FERRO__NET_SCORE, RATIO, WGCNA_INTER, Z_COMBINED",
            "expected_delta_cindex": "+0.008..+0.020",
            "status": "aktif (--add_paper_traits ile)"
        }, {
            "improvement": "Çok seviyeli stacking",
            "param": "Ridge + ElasticNet + LASSO + rank-avg meta-learner",
            "expected_delta_cindex": "+0.010..+0.025",
            "status": "aktif (use_stacking=True varsayılan)"
        }, {
            "improvement": "ETX+RSF OOF-optimize",
            "param": "optimize_ensemble_weights(ETX_oof, RSF_oof)",
            "expected_delta_cindex": "+0.003..+0.010",
            "status": "aktif"
        }, {
            "improvement": "ETX+RSF+GBM üçlü hybrid",
            "param": "w=[0.5,0.3,0.2]",
            "expected_delta_cindex": "+0.002..+0.008",
            "status": "aktif (GBM varsa)"
        }, {
            "improvement": "Stability prefilter",
            "param": "bag_frac>=0.60 öncelikli",
            "expected_delta_cindex": "+0.003..+0.010",
            "status": "aktif"
        }, {
            "improvement": "Adaptif meta-univariate eşik",
            "param": "frac→0.35, events→8 (az gen varsa)",
            "expected_delta_cindex": "stabilite",
            "status": "aktif"
        }, {
            "improvement": "RSF deeper trees",
            "param": "min_samples_split=max(2,min_leaf), leaf=2/3 grid",
            "expected_delta_cindex": "+0.002..+0.008",
            "status": "aktif"
        }])
        v35_summary.to_excel(w, sheet_name="v35_improvements", index=False)
    ...
    try:
        ...
        with pd.ExcelWriter(outdir/"publication"/"publication_summary.xlsx") as w:
            ...
            try:
                ...
                print(f"[WARN] v31 publication sheets failed: {_v31ex}")

        rep = []
        rep.append("# Manuscript-ready output summary (v33)\n")
        ...
        rep.append("## v33 Changes\n")
        rep.append("- ETX ince grid: mf=0.65..0.85, md=9..14, leaf=3..10, ne=260/320/400.\n")
        rep.append("- RSF: İLK KEZ çalıştırıldı (paper RSF+StepCox karşılığı). ne=300..600.\n")
        rep.append("- GBM: İLK KEZ çalıştırıldı. Konservatif grid (ne=100..800, lr=0.003..0.1).\n")
        ...
        rep.append("\n## Files\n")
        rep.append("- publication/publication_summary.xlsx (Table1-6 + Supp sheets)\n")
        rep.append("- publication/all_metrics_union.csv\n")
        ...
        print("[WARN] publication bundle failed:", type(_pub_ex).__name__)

    # v44-FIX-7 / v45: Methodological notes
    try:
        _rna_count_final = len([f for f in feats if str(f).startswith("RNA__")])
        _eo_method = ("Cox-baseline hazard (bireysel predicted survival olasılıkları) "
                      if LIFELINES_AVAILABLE else "KM risk-grup proxy (Cox mevcut değil)")
        _hist_filter_note = ""
        if _cohort_hist_filter:
            _hist_filter_note = (f"\n  Histoloji filtresi: {_cohort_hist_filter} "
                                 f"(mixed-histoloji kohortlar için ADC-only filtre uygulandı)")
        _notes = [
            "# v45 Methodological Notes — Methods Bölümü için\n\n",
            "## Gen seçimi pipeline\n",
            f"  Variance prefilter: top_{args.top_rna_var_prefilter} gen\n",
            f"  Meta-univariate: bagged (n_bags={args.n_bags}, bag_frac=0.8), "
            f"stage-adjusted={args.stage_adjusted_univariate}\n",
            f"  WGCNA filter: {args.use_wgcna_filter}\n",
            f"  Stability lock: min_freq={args.min_gene_stability}, cap={args.final_gene_cap}\n",
            f"  Son RNA gen sayısı: {_rna_count_final}\n\n",
            "## Validasyon\n",
            f"  İç validasyon: LOCO (Leave-One-Cohort-Out)\n",
            f"  Algorithm grid: 150 config, 9 family\n",
            f"  Ensemble: stacking (Ridge/ElasticNet/LASSO-Cox, OOF seçimi) + "
            f"diverse top-k={args.top_k_algos}\n",
            f"  External test kohortu/ları: {test}{_hist_filter_note}\n\n",
            "## İstatistiksel metrikler\n",
            "  Birincil: Uno's IPCW C-index (tau=Q75 event times, Uno 2011 Stat Med)\n",
            f"  Permütasyon testi: n_perm={args.n_perm} (H0: risk skoru survival'dan bağımsız)\n",
            f"  Bootstrap CI: n_boot={args.n_boot}, percentile method (2.5/97.5)\n",
            "  NRI/IDI: Pencina et al. 2008 Stat Med; "
            "NRI SE: bootstrap (n=500), IDI SE: bootstrap (n=500)\n",
            f"  E/O ratio: {_eo_method}\n",
            "  DCA: Simplified binary DCA (rank-normalized risk, fixed horizon). "
            "NOTE: Survival DCA (IPCW-weighted) önerilir; rcmdcheck veya dcurves paketi kullanın.\n",
            "  Kalibrasyon: Cox-on-risk baseline hazard ile observed vs predicted survival\n",
            "  PH testi: Schoenfeld residuals (lifelines.statistics.proportional_hazard_test)\n",
            "  Çok değişkenli Cox: stage-adjusted (CLIN__stage_num covariate)\n",
            "  RMST (v45): Restricted Mean Survival Time at 3yr and 5yr, bootstrap CI n=200. "
            "PH ihlali olan kohortlarda C-index'e alternatif birincil metrik olarak raporlanır.\n",
            "  Landmark analizi (v45): t=1, 2, 3 yıl eşiklerinde koşullu HR. "
            "PH ihlali olan kohortlarda (GSE68465) zamanla değişen etki belgelenir.\n\n",
            "## Sınırlamalar (Methods'ta belirtilmeli)\n",
            "  - DCA implementasyonu survival DCA değil, binary DCA (basitleştirilmiş)\n",
            "  - E/O ratio: Cox-baseline hazard proxy kullanılıyor\n",
            "  - GSE68465'te PH ihlali var (22/30 model); RMST ve landmark analizi ek olarak raporlandı\n",
            f"  - GSE30219 ADC-only filtreli kullanıldı (n≈85); full kohortu metodolojik tutarsızlık nedeniyle dışlandı\n"
            if "GSE30219" in str(test) else "",
            "  - Wet-lab doğrulama (qRT-PCR, IHC) yapılmadı — önerilir\n",
        ]
        (outdir / "logs" / "methodological_notes.txt").write_text(
            "".join([n for n in _notes if n]), encoding="utf-8")
        print("[v45] Methodological notes yazıldı: logs/methodological_notes.txt")
    except Exception as _mn_ex:
        print(f"[WARN v45] methodological notes failed: {_mn_ex}")

    print("\n[DONE] outputs:", outdir.resolve())


# v31 — ferroptosis prognostic signature pipeline (updated)
